...
// Guarda o ID do pokemon que esta sendo exibido nos detalhes
var _currentPokeId = null;

// Botao "Ver detalhes na Pokedex" — abre a pokedex02.html com aquele pokemon
function irParaPokedex() {
 if (_currentPokeId) {
 sessionStorage.setItem("RANKING_POKEMON_NAME", _currentPokeId);
 window.location.href = "pokedex02.html?id=" + _currentPokeId;
 }
}
...
function renderUsageList() {
 ...
 // Clicar leva direto à pokedex02.html já carregando este Pokémon
 return `
 <div class="usage-row" onclick="irParaPokedexDireto('${p.name}', ${id})">
...
// Navegação direta para a pokedex02 a partir do ranking
function irParaPokedexDireto(name, id) {
 sessionStorage.setItem('RANKING_POKEMON_NAME', name || id);
...
async function openDetail(name) {
 ...
 _currentPokeId = data.id;
 const btnPokedex = document.getElementById('btn-ver-pokedex');
 if (btnPokedex) btnPokedex.style.display = 'block';
